...
def get_progress_info():
    ...
    if progressbar:
        bar = progressbar.ProgressBar(widgets=[
            ...
            progressbar.Bar(), ' ',
            progressbar.FileTransferSpeed(unit='Bit'),
            ' (', progressbar.ETA(), ') ',
            ' (', progressbar.AdaptiveETA(format=u'AdaptiveETA: %(eta)s', format_zero=u'AdaptiveETA: 0:00:00'), ') ',
...
def run_main(parser, options, args, output=sys.stdout):
    ...
    if youtube:
        for index, video_path in enumerate(args):
            ...
            debug("Time elapsed: {0}".format(timedelta(seconds=end-start)))
